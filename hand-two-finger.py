...
while cap.isOpened():
    ...
    if results.multi_hand_landmarks:
        # Assume the first detected hand is the left hand and the second is the right hand
        if len(results.multi_hand_landmarks) == 2:
            left_hand_landmarks = results.multi_hand_landmarks[0]
            right_hand_landmarks = results.multi_hand_landmarks[1]

            # Get landmark positions for index fingertips.
            ...
            # Calculate distance between the index fingertips of both hands.
            index_finger_distance = calculate_distance(left_index_tip, right_index_tip)

            # Check for left click gesture.
            current_time = time.time()
            if index_finger_distance < 0.01 and current_time - last_click_time >= 0.5:  # 0.5-second delay
